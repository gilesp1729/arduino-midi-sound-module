...
class Dac final {
  ...
  static void setup() {
    ...
    // The port numbers here correspond to the pin numbers in the comment above.
    // They are different depending on the Arduino board being used.

    // TODO: Fix this to used board defines rather than CPU defines;.
#ifdef ARDUINO_AVR_UNO
    DDRD |= _BV(DDD5) | _BV(DDD6);                                  // Uno: Output PWM to DDD5 / DDD6
#elif defined(ARDUINO_AVR_MEGA2560)
    DDRG |= _BV(DDG5);                                              // Mega: Output PWM to DDG5 / DDB7
    DDRB |= _BV(DDB7);
#else
#error Unsupported board for PWM DAC
#endif
...
#ifdef ARDUINO_AVR_UNO
    DDRB |= _BV(DDB1) | _BV(DDB2);                      // Uno: Output PWM to DDB1 / DDB2
#elif defined(ARDUINO_AVR_MEGA2560)
    DDRB |= _BV(DDB5) | _BV(DDB6);                      // Mega: Output PWM to DDB5 / DDB6
#else
#error Unsupported board for PWM DAC
#endif
